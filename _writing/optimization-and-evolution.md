...
In machine learning, the loss is chosen to measure how well the model is performing. Roughly, it compares the true label to the predicted label and penalizes the prediction according to how wrong it is. For the much discussed large language models (LLMs), think of the "label" as being the next word in the sequence. For instance, given the input "Harry Potter is a," the predicted word "wizard" will score better than the word "toad." The model is trained by minimizing the loss over training data (e.g., a bunch of text from the internet), using optimization to efficiently tweak the parameters of the model until the loss is sufficiently small.
...
<img id='img-70' src="/assets/writing_images/yud_tweet.png">
...
First, evolution is not goal-oriented—there is nothing it's trying to do. It is not proceeding "in the direction of steepest descent" in the same way that gradient descent is (even if that phrase were to mean anything in the context of natural selection). Second, inclusive fitness is a more abstract notion than a mathematical loss function. It is constantly changing depending on the organism and its niche; a loss function is not. Third, loss functions in machine learning include the desired output. That is, they compare the predicted output with the target output. Inclusive fitness has no target output. So the means by which you're judging progress is different in the two cases.

### Hardware versus software

As Quintin Pope [points out](https://www.lesswrong.com/posts/wAczufCpMdaamF9fy/my-objections-to-we-re-all-gonna-die-with-eliezer-yudkowsky), the "optimization" of natural selection and the optimization in machine learning are working at different levels. Natural selection changes our genome, which gives rise to our brain structure—what connects to what, how many connections there are, the physical shape of our brains, and so on. Optimization in machine learning takes the neural network architecture as given and optimizes over the parameters of the network. The analogous process in evolution would be to optimize over a person's thoughts, which is distinct from what they're capable of thinking.

More generally, natural selection is an embodied process. It involves directly interacting with the world. The claim that (super)intelligence can be evolved in an artificial, digital world is not at all obvious. In fact, there is a direct conflict between evolution and optimization here: natural selection in humans first produced motor functionality and only then evolved higher cognitive capacities. Machine learning is attempting to go straight for the latter. Even if you believe this is possible (which I do), the argument as to _why_ it's possible cannot follow evolutionary logic.

### Novelty is punished

The final disanalogy comes from a discussion with [Jacob Springer](https://sprin.xyz/). Optimization in machine learning works, as we've (tediously) discussed, by optimizing over the training data. This implies that the model will be penalized for new "ideas" (sequences of words) that are not part of the training set, _regardless of whether they are true or false_. More precisely, it will be penalized for sequences of words whose conditional frequencies do not match those of the training set.
...
And here is Ilya Sutskever, chief scientist at OpenAI, on the Lunar Society Podcast [discussing](https://www.youtube.com/watch?v=Yf1o0TQzry8) the power of next token prediction:
...
These propositions are shown to be insubstantial by simply applying them to ourselves. Humans would be more successful in spreading our genes had we been as strong as bears, as fast as cheetahs, and able to predict the movement of every atom in the universe. Why aren't we these things? Because evolution by natural selection is constrained by existing organisms, working in a specific environment. Talking about the best possible set of behaviors to evolve is meaningless. So it is for evolution and so it is for optimization.
